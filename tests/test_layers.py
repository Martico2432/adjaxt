...
from adjaxt.config import RMSNormConfig, SwiGLUConfig, GQAAttnConfig, Qwen3AttnConfig, compute_rope_freqs, StandardAttnImplementation, PrecisionPolicy
...
def test_rms_norm_matches_hf(dtype, shape, key):
    ...
    # Configure policy based on test dtype
    p_dtype = jnp.bfloat16 if dtype == "bfloat16" else jnp.float32
    policy = PrecisionPolicy(param_dtype=p_dtype, compute_dtype=p_dtype, output_dtype=jnp.float32)
    cfg = RMSNormConfig(dim=shape[-1], eps=1e-6, precision=policy)
    
    w_np = rms_norm_init(key, cfg)

    if dtype == "bfloat16":
        x_j, w_j = jnp.asarray(x_np, jnp.bfloat16), jnp.asarray(w_np, jnp.bfloat16)
        x_t, w_t = torch.tensor(x_np).bfloat16(), torch.tensor(np.array(w_np, dtype=np.float32)).bfloat16()
        tol = 1e-2
    else:
        x_j, w_j = jnp.asarray(x_np), jnp.asarray(w_np)
        x_t, w_t = torch.tensor(x_np), torch.tensor(np.array(w_np, dtype=np.float32))
        tol = 1e-5
...
def test_swiglu_matches_hf(in_dim, hidden_dim, k):
    ...
    policy = PrecisionPolicy(param_dtype=jnp.float32, compute_dtype=jnp.float32, output_dtype=jnp.float32)
    cfg = SwiGLUConfig(in_dim, hidden_dim, precision=policy)
...
def test_gqa_attention_match(
    ...
    jax_cfg = GQAAttnConfig(
        implementation=StandardAttnImplementation.XLA,
        num_kv_groups=num_q_heads // num_kv_heads,
        is_causal=is_causal,
        precision=PrecisionPolicy(compute_dtype=jnp.float32, output_dtype=jnp.float32)
    )
    jax_out = gqa_attn(
...
def test_qwen3_attn_matches_pytorch(batch_size, seq_len, d, head_dim, num_heads, num_kv_heads):
    q_rms_conf = RMSNormConfig(dim=head_dim, eps=1e-6, precision=PrecisionPolicy(compute_dtype=jnp.float32, output_dtype=jnp.float32))
    k_rms_conf = RMSNormConfig(dim=head_dim, eps=1e-6, precision=PrecisionPolicy(compute_dtype=jnp.float32, output_dtype=jnp.float32))
