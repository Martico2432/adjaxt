...
def test_rms_norm_conversion(rng_key):
    cfg = RMSNormConfig(dim=64, eps=1e-6)
...
def test_swiglu_conversion(rng_key):
    cfg = SwiGLUConfig(in_dim=32, hidden_dim=64)
...
def test_gqa_attn_conversion(rng_key):
    cfg = GQAAttnConfig(
...
def test_qwen3_attn_conversion(rng_key):
    d_model = 64
    head_dim = 16
    seq_len = 8
    ...
    k1, _ = jax.random.split(rng_key)
    x_jax = jax.random.normal(k1, (2, seq_len, d_model), dtype=jnp.bfloat16)
    x_torch = _to_torch(x_jax, dtype=torch.bfloat16)
    
    out_jax = qwen3_attn(x_jax, w_jax, cfg)
    out_torch = torch_layer(x_torch)
    
    assert_allclose(out_jax, out_torch, rtol=1e-2, atol=1e-2)
...
def test_qwen3_moe_block_conversion(rng_key):
    d_model = 32
    hidden_dim = 64
    num_experts = 4
    top_k = 2
    ...
    # FIX: Cast to float32 for deterministic equivalence checks
    w_jax_f32 = jax.tree.map(lambda x: x.astype(jnp.float32) if hasattr(x, "astype") else x, w_jax)
    torch_layer = to_torch_module(qwen3_moe_block, cfg, weights=w_jax_f32).float()
    
    k1, _ = jax.random.split(rng_key)
    # FIX: Use float32 for inputs
    x_jax = jax.random.normal(k1, (2, 4, d_model), dtype=jnp.float32)
    x_torch = _to_torch(x_jax, dtype=torch.float32)
    
    out_jax = qwen3_moe_block(x_jax, w_jax_f32, cfg)
    out_torch = torch_layer(x_torch)
...
def test_qwen3_moe_layer_conversion(rng_key):
    d_model = 32
    head_dim = 8
    seq_len = 4
    ...
    mlp_conf = Qwen3MLPConfig(act_fn=jax.nn.silu, in_dim=d_model, hidden_dim=64)
    ...
    cfg = Qwen3MoELayerConfig(
        input_rms_conf=RMSNormConfig(dim=d_model, eps=1e-6),
        attn_conf=attn_conf,
        post_attn_rms_conf=RMSNormConfig(dim=d_model, eps=1e-6),
        moe_block_conf=moe_block_conf
    )
    
    w_jax = qwen3_moe_layer_init(rng_key, cfg)
    
    # 1. Cast JAX weights to float32
    w_jax_f32 = jax.tree.map(lambda x: x.astype(jnp.float32) if hasattr(x, "astype") else x, w_jax)
    
    # 2. Initialize torch layer with the float32 weights
    torch_layer = to_torch_module(qwen3_moe_layer, cfg, weights=w_jax_f32).float()
    
    k1, _ = jax.random.split(rng_key)
    
    # 3. Generate inputs as float32 instead of bfloat16
    x_jax = jax.random.normal(k1, (2, seq_len, d_model), dtype=jnp.float32)
    x_torch = _to_torch(x_jax, dtype=torch.float32)
    
    out_jax = qwen3_moe_layer(x_jax, w_jax_f32, cfg)
    out_torch = torch_layer(x_torch)
    
    # 4. Use tighter tolerances now that we are in f32
    assert_allclose(out_jax, out_torch, rtol=1e-4, atol=1e-4)
...
def test_qwen3_moe_model_conversion(rng_key, tie_word_embeddings):
    vocab_size = 128
    d_model = 32
    head_dim = 8
    seq_len = 4
    ...
    cfg = Qwen3MoEModelConfig(
        ...
        final_rms_conf=RMSNormConfig(dim=d_model, eps=1e-6),
    ...
    # Initialize weights in float32 for deterministic mathematical equivalence checks
    w_jax = qwen3_moe_model_init(rng_key, cfg)
    w_jax_f32 = jax.tree.map(lambda x: x.astype(jnp.float32) if hasattr(x, "astype") else x, w_jax)
    
    torch_model = to_torch_module(qwen3_moe_model, cfg, weights=w_jax_f32).float()
    
    tokens = np.random.randint(0, vocab_size, size=(2, seq_len))
    tokens_jax = jnp.array(tokens)
    tokens_torch = torch.from_numpy(tokens).long()
    
    logits_jax = qwen3_moe_model(tokens_jax, w_jax_f32, cfg)
