...
def test_qwen3_mlp_matches_pytorch(batch_size, seq_len, in_dim, hidden_dim):
    f32_policy = PrecisionPolicy(param_dtype=jnp.float32, compute_dtype=jnp.float32, output_dtype=jnp.float32)
    cfg = Qwen3MLPConfig(act_fn=jax.nn.silu, in_dim=in_dim, hidden_dim=hidden_dim, precision=f32_policy)
...
def test_qwen3_moe_block_matches_pytorch(d_model, hidden_dim, num_experts, top_k):
    f32_policy = PrecisionPolicy(param_dtype=jnp.float32, compute_dtype=jnp.float32, output_dtype=jnp.float32)
    mlp_cfg = Qwen3MLPConfig(act_fn=jax.nn.silu, in_dim=d_model, hidden_dim=hidden_dim, precision=f32_policy)
    ...
    with torch.no_grad():
        pt_moe.router.weight.copy_(torch.from_numpy(np.array(w_jax["router"]).T))
        for e in range(num_experts):
            pt_moe.experts[e].w_gate.weight.copy_(torch.from_numpy(np.array(w_jax["experts"]["w_gate"][e]).T))
            pt_moe.experts[e].w_up.weight.copy_(torch.from_numpy(np.array(w_jax["experts"]["w_up"][e]).T))
            pt_moe.experts[e].w_down.weight.copy_(torch.from_numpy(np.array(w_jax["experts"]["w_down"][e]).T))

    x_np = np.random.randn(2, 4, d_model).astype(np.float32)
    jax_out = qwen3_moe_block(jnp.array(x_np), w_jax, moe_cfg)
